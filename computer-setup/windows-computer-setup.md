...
1. Visit the official [Zoom Desktop Client Download page](https://zoom.us/download#client_4meeting).
2. Click on the download button or the link that matches your computer setup.
3. The installer will begin downloading automatically.
4. Once the download is complete, locate the installer file in your computer's Downloads folder.
5. Double-click on the installer file to launch the installation process.
6. Click on the "Sign Up" or "Sign In" button in the popup window that appears after the installation process.
...
2. Click on the download button that matches your computer setup
3. Run the downloaded installer file from your "Downloads" folder.
4. Follow the on-screen instructions to complete the installation process.
5. Once the installation is finished, launch the VSCode app.
6. (Optional) Pin the app to the taskbar for quick access.

---

## Windows Subsystem for Linux (WSL)

The Windows Subsystem for Linux (WSL) allows developers to run a GNU/Linux environment, which includes command-line tools, utilities, and applications, directly on Windows without the need for a virtual machine manager or dual-boot setup.
...
**NOTE: The ```wsl --install``` command should automatically start an Ubuntu session immediately - this is where you will set up your default unix user. If it doesn't follow the instructions in the terminal to start the session (should be something like `wsl.exe --install` to setup your user)**


1. The unix user will be the name of the user that you will be using in development. It does not need to match your windows user name - but should be easy to remember and recognise as you.

2. Setting the unix user password is critical as you will need it frequently in the future. Ensure its easy for *you* to remember!
...
4. Change default shell to zsh.This will make your terminal look nicer, and give you some extra useful information.
...
2. Install Node to install nvm please run the following command in your terminal

```bash
 curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.35.3/install.sh | zsh
 ```
3. Run the following command to install the latest long term support version of Node.js.
  
   ```bash
   nvm install --lts
   ```
4. Run the following command to set up the default node alias
    ```bash
